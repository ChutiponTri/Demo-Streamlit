...
def get_username_list():
    return list(FireBase().get_database("customers/Username"))

@st.cache_data
def get_sheetnames(table_names):
    return list(FireBase().get_database(table_names).keys())

@st.cache_data
def get_storage():
    return FireBase().get_database("storage")

@st.cache_data
def get_overview(overview_path):
    return FireBase().get_database(overview_path)

@st.cache_data
def get_raw_data(raw_data_path):
    return FireBase().get_database(raw_data_path)

@st.cache_data
def get_user_history():
    return FireBase().get_database("current")

@st.cache_data
def create_fig(df, label):
    fig, ax = plt.subplots()
    ax.plot(df[label], label=label)
    ax.legend(bbox_to_anchor=(0.95, 1.14), loc='upper center')
    ax.set_xlabel("Data")
...
class Stream():
    def __init__(self):
        ...
        self.user_list = get_username_list()
        ...
        try:
            ...
            self.sheetname_list = get_sheetnames(table_names)
    ...
    def tab1_ui(self):
        with self.game:
            ...
            if self.side_select != "Please Select The Game":
                ...
                # self.score_data = self.firebase.get_database("storage")
                if len(self.score_data) != 0:
                    df = pd.DataFrame(self.score_data)[score_order]
    ...
    def tab2_ui(self):
        with self.score:
            ...
            score_df = pd.DataFrame(self.score_data)[score_order]   
    ...
    def tab3_ui(self):
        with self.overview:
            ...
            try:
                ...
                overview = get_overview(overview_path)
                if len(overview) != 0:
                    # Create DataFrame
    ...
    def tab4_ui(self):
        with self.motion:
            ...
            if datasheet != "Please Select Data":
                ...
                # self.raw_data = self.firebase.get_database(raw_data_path)
